...
import com.atlandes.auth.shiro.session.SessionMetaInfo;
...
public class SimpleAuthFilter extends AccessControlFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request,
        ...
        SessionMetaInfo sessionStatus = (SessionMetaInfo) session.getAttribute(MercurySessionManager.SESSION_STATUS);
        if (null != sessionStatus && !sessionStatus.isOnlineStatus()) {
            //判断是不是Ajax请求
            if (ShiroFilterUtils.isAjax(request)) {
                ShiroLogUtils.debug(getClass(), "当前用户已经被踢出，并且是Ajax请求！");
